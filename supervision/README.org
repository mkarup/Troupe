...
+In fact, even if the termination signal that is sent from P3 to P2 is tainted by the context and propagated to the termination signal sent from P2 to P1, it will leak information.+


Modify the semantics to address/mitigate the issues identified with the basic API.
